...
use super::indexes::*;
use crate::{executor::predicate::*, executor::query::*, fileio::pageio::*, util::row::*};
...
impl LeafIndexPage {
    ...
    pub fn load_from_table(
        ...
        for i in 0..num_values {
            ...
            let index_value: LeafIndexValue =
    ...
    pub fn add_pointer_to_row(&mut self, rowinfo: &RowInfo) -> Result<bool, String> {
        if !self.has_room() {
            return Ok(false);
        }
    ...
    pub fn add_pointer_to_leaf_value(
        ...
        for (i, (key, _)) in self.indexes.clone().iter().enumerate().sorted() {
            if compare_indexes(&key, &index_key) == KeyComparison::Greater {
                self.indexes
                    .insert(i, (index_key.clone(), leaf_value.clone()));
                did_insert = true;
                break;
            }
        }
    ...
    pub fn remove_pointer_to_row(&mut self, rowinfo: &RowInfo) -> Result<(), String> {
        if self.is_empty() {
    ...
    pub fn get_row_locations_from_key(
        ...
        if self
            ...
            .iter()
            .map(|(key, _)| key)
            .find(|key| compare_indexes(key, index_key) == KeyComparison::Equal)
            .is_some()
        {
    ...
    pub fn get_all_key_values(&self) -> Vec<(IndexKey, LeafIndexValue)> {
        self.indexes.clone()
    }
    ...
    pub fn has_room(&self) -> bool {
        ...
        if combined_size + self.key_size as usize + LeafIndexValue::size()
    ...
    pub fn get_max_index_pointers_per_page(index_key_type: &IndexKeyType) -> usize {
        let idx_and_value_size: usize =
            get_index_key_type_size(index_key_type) + LeafIndexValue::size();
...
mod tests {
    ...
    fn test_read_write_index_value() {
        ...
        let index_value1: LeafIndexValue = LeafIndexValue {
            pagenum: 4,
            rownum: 182,
        };
        let index_value2: LeafIndexValue = LeafIndexValue {
            pagenum: 50,
            rownum: 6,
        };
        let index_value3: LeafIndexValue = LeafIndexValue {
            pagenum: 235,
            rownum: 67,
        };
    ...
    fn test_read_write_index_key_values() {
        ...
        let index_value4: LeafIndexValue = LeafIndexValue {
            pagenum: 235,
            rownum: 201,
        };
    ...
    fn test_leaf_load_from_table() {
        ...
        LeafIndexPage::write_index_key(&index_key1, &index_key_type, &mut leaf_page.page, 0)
            .unwrap();
        LeafIndexPage::write_index_key(&index_key2, &index_key_type, &mut leaf_page.page, 1)
            .unwrap();
        LeafIndexPage::write_index_value(&index_value1, &index_key_type, &mut leaf_page.page, 0)
            .unwrap();
        LeafIndexPage::write_index_value(&index_value2, &index_key_type, &mut leaf_page.page, 1)
            .unwrap();
        LeafIndexPage::write_index_value(&index_value3, &index_key_type, &mut leaf_page.page, 2)
            .unwrap();
        ...
        assert_eq!(loaded_leaf_page.index_id, index_id);
        assert_eq!(loaded_leaf_page.index_key_type, index_key_type);
        assert_eq!(loaded_leaf_page.pagenum, leaf_page_num);
    ...
    fn test_get_row_locations_from_key() {
        let (table, leaf_pagenum, index_key_type, index_id, index_keys, index_values) =
            create_testing_table_and_leaf_page();
        ...
        assert_eq!(
            leaf_page
                .get_row_locations_from_key(&index_keys[0])
                .unwrap(),
            vec![index_values[0].to_row_location()]
        );
        assert_eq!(
            leaf_page
                .get_row_locations_from_key(&index_keys[1])
                .unwrap(),
            vec![index_values[1].to_row_location()]
        );
        assert_eq!(
            leaf_page
                .get_row_locations_from_key(&index_keys[2])
                .unwrap(),
            vec![index_values[2].to_row_location()]
        );
        assert_eq!(
            leaf_page
                .get_row_locations_from_key(&index_keys[3])
                .unwrap(),
        ...
        assert_eq!(
            leaf_page
                .get_row_locations_from_key(&vec![
                    Value::I32(1),
    ...
    fn test_get_row_locations_from_key_range() {
        let (table, leaf_pagenum, index_key_type, index_id, _, index_values) =
            create_testing_table_and_leaf_page();
        ...
        assert_eq!(
            leaf_page
                .get_row_locations_matching_expr(
                    &Expr::BinaryOp {
                ...
                .unwrap(),
            vec![
                index_values[2].to_row_location(),
                index_values[3].to_row_location()
            ]
        );
        ...
        assert_eq!(
            leaf_page
                ...
                .unwrap(),
            vec![
                index_values[1].to_row_location(),
                index_values[2].to_row_location(),
                index_values[3].to_row_location()
            ]
        );
    ...
    fn test_get_row_locations_from_key_range2() {
        let (table, leaf_pagenum, index_key_type, index_id, _, index_values) =
            create_testing_table_and_leaf_page2();
    ...
    fn create_testing_table_and_leaf_page() -> (
        Table,
        u32,
        IndexKeyType,
        IndexID,
        ...
        LeafIndexPage::write_index_key(&index_key2, &index_key_type, &mut leaf_page.page, 1)
            .unwrap();
        LeafIndexPage::write_index_key(&index_key3, &index_key_type, &mut leaf_page.page, 2)
            .unwrap();
        LeafIndexPage::write_index_key(&index_key4, &index_key_type, &mut leaf_page.page, 3)
            .unwrap();
        LeafIndexPage::write_index_value(&index_value1, &index_key_type, &mut leaf_page.page, 0)
            .unwrap();
        LeafIndexPage::write_index_value(&index_value2, &index_key_type, &mut leaf_page.page, 1)
        ...
        LeafIndexPage::write_index_value(&index_value3, &index_key_type, &mut leaf_page.page, 2)
            .unwrap();
        LeafIndexPage::write_index_value(&index_value4, &index_key_type, &mut leaf_page.page, 3)
            .unwrap();

        leaf_page
            .indexes
            .push((index_key1.clone(), index_value1.clone()));
        leaf_page
            .indexes
            .push((index_key2.clone(), index_value2.clone()));
        leaf_page
            .indexes
            .push((index_key3.clone(), index_value3.clone()));
        leaf_page
            .indexes
            .push((index_key4.clone(), index_value4.clone()));
    ...
    /// Creates a testing table and leaf page with 4 index keys and values.
    fn create_testing_table_and_leaf_page2() -> (
        Table,
        u32,
        IndexKeyType,
        IndexID,
