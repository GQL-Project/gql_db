...
use super::branches::{Branches, BranchNode};
...
pub fn log () -> Result<(), String> {
    ...
    let branch_node = branch_instance
        .get_branch_node_from_head(&branch_name, &branches_from_head).unwrap();
    ...
    for node in branch_nodes {
        let commit = get_db_instance()?.get_commit_file_mut().fetch_commit(&node.commit_hash)?;
